...
#if !defined(HEADLESS) && !defined(__ANDROID__)
...
#ifdef HEADLESS
    if (mid != NULL) {
		bounds = (*env)->NewObject(env, clazz, mid, 0, 0, GLOBAL_WIDTH, GLOBAL_HEIGHT);
	}
    return bounds;
#else
...
{
#ifdef HEADLESS
	return X11GD_CreateDisplayMode(env, GLOBAL_WIDTH, GLOBAL_HEIGHT /* mode->width, mode->height */, BIT_DEPTH_MULTI, (int)(/* rate */ 60 +.2));
#else
    XRRScreenConfiguration *config;
