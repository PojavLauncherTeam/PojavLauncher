...
public class VncCanvas extends ImageView {
	...
	private VncCanvasActivity vncActivity;
...
*/
	 ...
	 */
	void initializeVncCanvas(VncCanvasActivity activity, ConnectionBean bean, final Runnable setModes) {
		vncActivity = activity;
		connection = bean;
		this.pendingColorModel = COLORMODEL.valueOf(bean.getColorModel());

		// Startup the RFB thread with a nifty progess dialog
		final ProgressDialogCompat pd = ProgressDialogCompat.show(getContext(), "Connecting...", "Establishing handshake.\nPlease wait...", true, false); /* true, new DialogInterface.OnCancelListener() {
			@Override
				...
				try {
					...
					if (maintainConnection) {
						...
						if (e instanceof OutOfMemoryError) {
							...
							if (vncActivity.ignoreDisconnect) return;
							handler.post(new Runnable() {
								public void run() {
	...
	public void processNormalProtocol(final Context context, ProgressDialogCompat pd, final Runnable setModes) throws Exception {
		try {
  ...
  int[] handleZRLERectPalette = new int[128];
	...
	class ZoomInputHandler extends AbstractGestureInputHandler {
		...
		ZoomInputHandler() {
			super(vncActivity);
			keyHandler = new DPadMouseKeyHandler(vncActivity,handler);
		}
		...
		public boolean onTrackballEvent(MotionEvent evt) {
			return vncActivity.trackballMouse(evt);
		}
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
							   float velocityY) {
			vncActivity.showZoomer(false);
			vncActivity.panner.start(-(velocityX / FLING_FACTOR),
		...
		public boolean onTouchEvent(MotionEvent e) {
			if (dragMode) {
				...
				if (e.getAction() == MotionEvent.ACTION_UP)
					dragMode = false;
				return processPointerEvent(e, true);
			} else
		...
		public void onLongPress(MotionEvent e) {
			vncActivity.showZoomer(true);
			BCFactory.getInstance().getBCHaptic().performLongPressHaptic(VncCanvas.this);
			dragMode = true;
			processPointerEvent(VncCanvas.this.changeTouchCoordinatesToFullFrame(e), true);
		}
		...
		public boolean onSingleTapConfirmed(MotionEvent e) {
			...
			return processPointerEvent(e, false);
		}
		...
		public boolean onDoubleTap(MotionEvent e) {
			changeTouchCoordinatesToFullFrame(e);
			processPointerEvent(e, true, true);
			e.setAction(MotionEvent.ACTION_UP);
			return processPointerEvent(e, false, true);
		}
	...
	public class TouchpadInputHandler extends AbstractGestureInputHandler {
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2,
			...
			else
			{
				...
				float deltaX = -distanceX *getScale();
				float deltaY = -distanceY *getScale();
				deltaX = fineCtrlScale(deltaX);
				deltaY = fineCtrlScale(deltaY);

				// compute the absolution new mouse pos on the remote site.
				float newRemoteX = mouseX + deltaX;
				float newRemoteY = mouseY + deltaY;
				...
				if (dragMode) {
					...
					return processPointerEvent(e2, true);
				} else {
					e2.setLocation(newRemoteX, newRemoteY);
					processPointerEvent(e2, false);
				}
			}
			return true;
		}
		...
		public boolean onTouchEvent(MotionEvent e) {
			if (dragMode) {
				// compute the relative movement offset on the remote screen.
				float deltaX = (e.getX() - dragX) *getScale();
				float deltaY = (e.getY() - dragY) *getScale();
				dragX = e.getX();
				dragY = e.getY();
				deltaX = fineCtrlScale(deltaX);
				deltaY = fineCtrlScale(deltaY);
		...
		public boolean onSingleTapConfirmed(MotionEvent e) {
			...
			processPointerEvent(e, true, multiTouch||cameraButtonDown);
			e.setAction(MotionEvent.ACTION_UP);
			return processPointerEvent(e, false, multiTouch||cameraButtonDown);
		}
