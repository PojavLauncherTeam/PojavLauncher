...
public final class ImageIO {
    ...
    public static void scanForPlugins() {
		...
		System.out.println("Warning: Calling an unsupported method javax.imageio.ImageIO.scanForPlugins()");
    }
    ...
    public static boolean write(RenderedImage im, String formatName, ImageOutputStream output)
		...
		*/
		...
		if (output instanceof FileImageOutputStream && im instanceof BufferedImage) {
			int[] pixels = ((BufferedImage) im).getRGB(0, 0, im.getWidth(), im.getHeight(), null, 0, im.getWidth());
			Bitmap androidBitmap = Bitmap.createBitmap(pixels, im.getWidth(), im.getHeight(), Bitmap.Config.ARGB_8888);
			FileOutputStream fOutput = new FileOutputStream(((FileImageOutputStream) output).getFile());
			boolean canSave = androidBitmap.compress(Bitmap.CompressFormat.PNG, 100, fOutput);
			fOutput.close();
			return canSave;
		}
        return false;
