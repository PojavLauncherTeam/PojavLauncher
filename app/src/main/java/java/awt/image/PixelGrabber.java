...
public class PixelGrabber implements ImageConsumer {
        ...
        case DATA_TYPE_BYTE:
            if(!isRGB && cm == model){
                for(int y = 0; y < srcH; y++){
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(pixels, srcOff, bData, realOff, srcW);
                    srcOff += srcScan;
                    realOff += scanline;
                }
                break;
    ...
    public void setPixels(int srcX, int srcY, int srcW, int srcH,
        ...
        case DATA_TYPE_INT:
            if(cm == model){
                for(int y = 0; y < srcH; y++){
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(pixels, srcOff, iData, realOff, srcW);
                    srcOff += srcScan;
                    realOff += scanline;
                }
                break;
