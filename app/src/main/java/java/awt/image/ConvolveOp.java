    ...
    private int slowFilter(Raster src, WritableRaster dst) {
        try {
            ...
            if (this.edgeCond == ConvolveOp.EDGE_NO_OP) {
                ...
                net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(pixels, start, newPixels, start, length);
                // bottom
                ...
                for (int i = yOrigin; i < (srcHeight - (kHeight - yOrigin - 1)); i++) {
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(pixels, start, newPixels, start, length);
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(pixels, start1, newPixels, start1, length1);
                    start += rowLength;
                    start1 += rowLength;
                }
