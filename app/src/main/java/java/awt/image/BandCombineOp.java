...
public class BandCombineOp implements RasterOp {
    ...
    public BandCombineOp(float matrix[][], RenderingHints hints) {
        ...
        for (int i = 0; i < mxHeight; i++) {
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(matrix[i], 0, this.matrix[i], 0, mxWidth);
        }
    ...
    public final float[][] getMatrix() {
        ...
        for (int i = 0; i < mxHeight; i++) {
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(matrix[i], 0, res[i], 0, mxWidth);
        }
