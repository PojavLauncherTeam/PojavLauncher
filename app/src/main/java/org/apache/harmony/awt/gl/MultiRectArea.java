            ...
            while (bottomIndex < lastCount || pointIndex < pointCount) {
                ...
                if (deleteRect) {
                    if (bottomIndex < bottomCount - 1) {
                        net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(bottom, bottomIndex + 1, bottom, bottomIndex, bottomCount - bottomIndex - 1);
                        rectIndex -= 4;
                    }
                    bottomCount--;
                    lastCount--;
...
/*
            ...
            while(i < cash[0]) {
                if (rect[cash[i] + 3] >= y1 - 1) {
                    if (i > 1) {
                        net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(cash, i, cash, 1, cash[0] - i);
                    }
                    break;
                }
                i++;
                ...
                if (rect[index] == x1 && rect[index + 2] == x2) {
                    ...
                    while(pos < cash[0]) {
                        if (rect[index + 3] <= rect[cash[i] + 3]) {
                            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(cash, i + 1, cash, i, pos - i);
                            break;
                        }
                        i++;
                    }
            ...
            while(i < length) {
                if (y2 <= rect[cash[i] + 3]) {
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(cash, i, cash, i + 1, length - i);
                    break;
                }
                i++;
            }
...
//              addRectCashed(rect[i++], rect[i++], rect[i++], rect[i++]);
    ...
    class Iterator implements PathIterator {
        ...
        Iterator(MultiRectArea mra, AffineTransform t) {
            ...
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(mra.rect, 1, rect, 0, rect.length);
            this.t = t;
        }
    ...
    public MultiRectArea(MultiRectArea mra) {
        if (mra == null) {
            ...
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(mra.rect, 0, rect, 0, mra.rect.length);
    ...
    void resort() {
        ...
        for(int i = 1; i < rect[0]; i += 4) {
            ...
            if (k != i) {
                net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(rect, i, buf, 0, 4);
                net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(rect, k, rect, i, 4);
                net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(buf, 0, rect, k, 4);
            }
        }
        invalidate();
    }
    ...
    protected void setRect(int[] buf, boolean copy) {
        if (copy) {
            ...
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(buf, 0, rect, 0, buf.length);
        } else {
            rect = buf;
        }
        invalidate();
