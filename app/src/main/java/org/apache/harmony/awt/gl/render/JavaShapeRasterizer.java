...
//              System.out.println("test");
    ...
    */
        ...
        static int excludeEmpty(int[] points, int length) {
            ...
            while(i < length) {
                if (points[i] <= points[i + 1]) {
                    ...
                    length -= 2;
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(points, i + 2, points, i, length - i);
                }
            }
            return length;
        }
        ...
        static int union(int[] points, int length) {
            ...
            while(i < length - 1) {
                if (points[i] < points[i - 1]) {
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(points, i + 1, points, i - 1, length - i - 1);
                    length -= 2;
                } else
                if (points[i] >= points[i + 1] - 1) {
                    net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(points, i + 2, points, i, length - i - 2);
                    length -= 2;
                } else {
                    i += 2;
                }
    ...
    int[] checkBufSize(int[] buf, int size) {
        if (size == buf.length) {
            ...
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(buf, 0, tmp, 0, buf.length);
            buf = tmp;
        }
        return buf;
    }
    ...
    void init() {
        ...
        net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(edgesY, 0, edgesYS, 0, edgesCount);
        // Create edgesDY
            ...
            if (y == nextY) {
                ...
                while(i > 0) {
                    ...
                    if (activeYEnd[i] == y) {
                        ...
                        if (length != 0) {
                            int pos = i + 1;
                            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(activeX, pos, activeX, i, length);
                            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(activeYEnd, pos, activeYEnd, i, length);
                            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(activeXStep, pos, activeXStep, i, length);
                            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(activeDY, pos, activeDY, i, length);
                            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(activeExt, pos, activeExt, i, length);
                        }
                    }
                }
