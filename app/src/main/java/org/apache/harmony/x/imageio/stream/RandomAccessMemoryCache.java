...
public final class RandomAccessMemoryCache {
    ...
    public void putData(byte[] buffer, int offset, int count, long pos) {
        ...
        while (count > 0) {
            ...
            net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(buffer, offset, block, blockOffset, toCopy);
            pos += toCopy;
            count -= toCopy;
            offset += toCopy;
        }
    ...
    public int getData(byte[] buffer, int offset, int count, long pos) {
        ...
        net.kdt.pojavlaunch.SystemCrackResolver.arraycopy(block, (int)(pos & BLOCK_MASK), buffer, offset, nbytes);
